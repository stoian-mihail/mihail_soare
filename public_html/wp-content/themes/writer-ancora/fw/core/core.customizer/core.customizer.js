// Writer Ancora Customizer script
...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.writer_ancora_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.writer_ancora_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && writer_ancora_get_cookie('to_demo_fixed')==1) writer_ancora_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.writer_ancora_options_field_info .to_demo_pin', function(e) {
		writer_ancora_options_fix_demo(jQuery(this).parents('.writer_ancora_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	writer_ancora_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.writer_ancora_options_input,.writer_ancora_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.writer_ancora_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		writer_ancora_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.writer_ancora_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.writer_ancora_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .writer_ancora_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.writer_ancora_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? WRITER_ANCORA_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.writer_ancora_options_tab').find('> .writer_ancora_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="writer_ancora_options_schemes_order" class="writer_ancora_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function writer_ancora_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.writer_ancora_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.writer_ancora_options').find('>.writer_ancora_options_field_info');
...
function writer_ancora_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.writer_ancora_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	writer_ancora_message_confirm(WRITER_ANCORA_STORAGE['to_strings']['scheme_delete_confirm'], WRITER_ANCORA_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'writer_ancora_options_scheme_delete',
			nonce: WRITER_ANCORA_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.writer_ancora_options_tab').find('> .writer_ancora_options_schemes_order').val()
		};
		setTimeout(function(){
			writer_ancora_message_info(WRITER_ANCORA_STORAGE['to_strings']['recompile_styles'], WRITER_ANCORA_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(WRITER_ANCORA_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				writer_ancora_message_success(WRITER_ANCORA_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+WRITER_ANCORA_STORAGE['to_strings']['reload_page'], WRITER_ANCORA_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				writer_ancora_message_warning(WRITER_ANCORA_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, WRITER_ANCORA_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function writer_ancora_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.writer_ancora_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	writer_ancora_message_confirm(WRITER_ANCORA_STORAGE['to_strings']['scheme_copy_confirm'], WRITER_ANCORA_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'writer_ancora_options_scheme_copy',
			nonce: WRITER_ANCORA_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.writer_ancora_options_tab').find('> .writer_ancora_options_schemes_order').val()
		};
		...
		jQuery.post(WRITER_ANCORA_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				writer_ancora_message_success(WRITER_ANCORA_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+WRITER_ANCORA_STORAGE['to_strings']['reload_page'], WRITER_ANCORA_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				writer_ancora_message_warning(WRITER_ANCORA_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, WRITER_ANCORA_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function writer_ancora_options_fix_demo(info) {
	var info2 = info.parents('.writer_ancora_options').toggleClass('to_demo_fixed').find('>.writer_ancora_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.writer_ancora_options').append(clonedObj);
		info2 = info.parents('.writer_ancora_options').find('>.writer_ancora_options_field_info');
	}
	writer_ancora_set_cookie('to_demo_fixed', jQuery('.writer_ancora_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function writer_ancora_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.writer_ancora_options_field_info').data('scheme');
