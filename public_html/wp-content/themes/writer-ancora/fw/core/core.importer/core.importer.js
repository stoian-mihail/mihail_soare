...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		writer_ancora_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function writer_ancora_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: WRITER_ANCORA_STORAGE['ajax_nonce'],
		action: 'writer_ancora_importer_start_import',
	...
	jQuery.post(WRITER_ANCORA_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '') {
			...
			// Do next action
			if (idx < steps.length) {
				writer_ancora_importer_do_action(steps, idx);
			} else {
