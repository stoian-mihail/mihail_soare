...
// Theme init
if (!function_exists('writer_ancora_widget_recent_reviews_theme_setup')) {
	add_action( 'writer_ancora_action_before_init_theme', 'writer_ancora_widget_recent_reviews_theme_setup', 1 );
	function writer_ancora_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('writer_ancora_action_shortcodes_list',	'writer_ancora_widget_recent_reviews_reg_shortcodes');
		if (function_exists('writer_ancora_exists_visual_composer') && writer_ancora_exists_visual_composer())
			add_action('writer_ancora_action_shortcodes_list_vc','writer_ancora_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('writer_ancora_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'writer_ancora_widget_recent_reviews_load' );
	function writer_ancora_widget_recent_reviews_load() {
		register_widget('writer_ancora_widget_recent_reviews');
	}
}

// Widget Class
class writer_ancora_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'writer-ancora'));
		parent::__construct( 'writer_ancora_widget_recent_reviews', esc_html__('Writer - Recent reviews', 'writer-ancora'), $widget_ops );

		// Add thumb sizes into list
		writer_ancora_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 125, 'h' => 125, 'title'=>esc_html__('Widgets ', 'writer-ancora') ) );
	}
	...
	function widget($args, $instance) {
		...
		$post_rating = 'writer_ancora_reviews_avg'.(writer_ancora_get_theme_option('reviews_first')=='author' ? '' : '2');
...
*/
if ( !function_exists( 'writer_ancora_sc_widget_recent_reviews' ) ) {
	function writer_ancora_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'writer_ancora_widget_recent_reviews';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, writer_ancora_prepare_widgets_args(writer_ancora_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('writer_ancora_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	writer_ancora_require_shortcode("trx_widget_recent_reviews", "writer_ancora_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('writer_ancora_widget_recent_reviews_reg_shortcodes_vc')) {
	function writer_ancora_widget_recent_reviews_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Reviews", 'writer-ancora'),
				"description" => wp_kses( __("Insert recent reviews list with thumbs and post's meta", 'writer-ancora'), writer_ancora_storage_get('allowed_tags') ),
