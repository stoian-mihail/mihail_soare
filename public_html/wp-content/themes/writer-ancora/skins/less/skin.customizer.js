...
function writer_ancora_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = WRITER_ANCORA_STORAGE['co_add_params']!=undefined ? WRITER_ANCORA_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#writer_ancora-customizer-styles-'+option).length > 0 ? jQuery('#writer_ancora-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="writer_ancora-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
