...
if ( !function_exists( 'writer_ancora_template_list_theme_setup' ) ) {
	add_action( 'writer_ancora_action_before_init_theme', 'writer_ancora_template_list_theme_setup', 1 );
	function writer_ancora_template_list_theme_setup() {
		writer_ancora_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'writer-ancora')
			));
	}
}

// Template output
if ( !function_exists( 'writer_ancora_template_list_output' ) ) {
	function writer_ancora_template_list_output($post_options, $post_data) {
