...
if ( !function_exists( 'writer_ancora_template_events_1_theme_setup' ) ) {
	add_action( 'writer_ancora_action_before_init_theme', 'writer_ancora_template_events_1_theme_setup', 1 );
	function writer_ancora_template_events_1_theme_setup() {
		writer_ancora_add_template(array(
			'layout' => 'events-1',
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'writer-ancora'),
...
// Template output
if ( !function_exists( 'writer_ancora_template_events_1_output' ) ) {
	function writer_ancora_template_events_1_output($post_options, $post_data) {
		$show_title = true;
							...
							if ($post_data['post_excerpt']) {
								echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
										...
										: '<p>'.trim(writer_ancora_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																										? $post_options['descr'] 
																										: writer_ancora_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
							}
							if (!empty($post_data['post_link']) && !writer_ancora_param_is_off($post_options['readmore'])) {
