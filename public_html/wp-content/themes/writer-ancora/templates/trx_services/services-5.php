...
if ( !function_exists( 'writer_ancora_template_services_5_theme_setup' ) ) {
	add_action( 'writer_ancora_action_before_init_theme', 'writer_ancora_template_services_5_theme_setup', 1 );
	function writer_ancora_template_services_5_theme_setup() {
		writer_ancora_add_template(array(
			'layout' => 'services-5',
			'template' => 'services-5',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 5/', 'writer-ancora'),
...
// Template output
if ( !function_exists( 'writer_ancora_template_services_5_output' ) ) {
	function writer_ancora_template_services_5_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					$html = writer_ancora_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
