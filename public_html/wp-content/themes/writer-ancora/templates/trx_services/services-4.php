...
if ( !function_exists( 'writer_ancora_template_services_4_theme_setup' ) ) {
	add_action( 'writer_ancora_action_before_init_theme', 'writer_ancora_template_services_4_theme_setup', 1 );
	function writer_ancora_template_services_4_theme_setup() {
		writer_ancora_add_template(array(
			'layout' => 'services-4',
			'template' => 'services-4',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 4/', 'writer-ancora')
		));
	}
}

// Template output
if ( !function_exists( 'writer_ancora_template_services_4_output' ) ) {
	function writer_ancora_template_services_4_output($post_options, $post_data) {
		$show_title = true;
