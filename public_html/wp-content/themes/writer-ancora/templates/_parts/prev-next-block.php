...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = writer_ancora_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo 'single-portfolio-fullscreen'==$page_style ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''; ?>>
			...
			$args = writer_ancora_query_add_posts_and_cats($args, '', $post_data['post_type'], 
			...
			if ( $prev ) {
				...
				$desc = writer_ancora_strshort($prev->post_title, 30);
				?>
			...
			if ( $next ) {
				...
				$desc = writer_ancora_strshort($next->post_title, 30);
				?>
