...
if ( !function_exists( 'writer_ancora_template_news_excerpt_theme_setup' ) ) {
	add_action( 'writer_ancora_action_before_init_theme', 'writer_ancora_template_news_excerpt_theme_setup', 1 );
	function writer_ancora_template_news_excerpt_theme_setup() {
		writer_ancora_add_template(array(
			'layout' => 'news-excerpt',
			'template' => 'news-excerpt',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Excerpt/', 'writer-ancora'),
...
// Template output
if ( !function_exists( 'writer_ancora_template_news_excerpt_output' ) ) {
	function writer_ancora_template_news_excerpt_output($post_options, $post_data) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(writer_ancora_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: writer_ancora_get_custom_option('post_excerpt_maxlength')
																)
											);
						}
					}
