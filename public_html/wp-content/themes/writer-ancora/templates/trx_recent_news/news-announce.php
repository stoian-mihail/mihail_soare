...
if ( !function_exists( 'writer_ancora_template_news_announce_theme_setup' ) ) {
	add_action( 'writer_ancora_action_before_init_theme', 'writer_ancora_template_news_announce_theme_setup', 1 );
	function writer_ancora_template_news_announce_theme_setup() {
		writer_ancora_add_template(array(
			'layout' => 'news-announce',
			'template' => 'news-announce',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Announce/', 'writer-ancora')
		));

		// Add thumb sizes into list
		writer_ancora_add_thumb_sizes( array( 'thumb_slug' => 'full', 'add_image_size' => false, 'w' => 1170, 'h' => 659 ) );
		writer_ancora_add_thumb_sizes( array( 'thumb_slug' => 'big', 'add_image_size' => false, 'w' => 760, 'h' => 428 ) );
		writer_ancora_add_thumb_sizes( array( 'thumb_slug' => 'medium', 'add_image_size' => false, 'w' => 760, 'h' => 214 ) );
		writer_ancora_add_thumb_sizes( array( 'thumb_slug' => 'small', 'add_image_size' => false, 'w' => 370, 'h' => 209 ) );
	}
}

// Template output
if ( !function_exists( 'writer_ancora_template_news_announce_output' ) ) {
	function writer_ancora_template_news_announce_output($post_options, $post_data) {
		...
		$post_data['post_thumb'] = writer_ancora_get_resized_image_tag($post_data['post_attachment'], $thumb_sizes['w'], $post_data['post_type']=='product' && writer_ancora_get_theme_option('crop_product_thumb')=='no' ? null :  $thumb_sizes['h']);
