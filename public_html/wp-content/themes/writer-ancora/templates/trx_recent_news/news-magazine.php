...
if ( !function_exists( 'writer_ancora_template_news_magazine_theme_setup' ) ) {
	add_action( 'writer_ancora_action_before_init_theme', 'writer_ancora_template_news_magazine_theme_setup', 1 );
	function writer_ancora_template_news_magazine_theme_setup() {
		writer_ancora_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'writer-ancora'),
...
// Template output
if ( !function_exists( 'writer_ancora_template_news_magazine_output' ) ) {
	function writer_ancora_template_news_magazine_output($post_options, $post_data) {
		...
		?><article id="post-<?php echo esc_html($post_data['post_id']); ?>" 
			...
			if ($number <= $featured) {
					...
					if ($post_data['post_protected']) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(writer_ancora_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
				...
				<div class="post_footer entry-footer">
					...
					<?php
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						$post_options['counters'] = 'views,comments,edit,captions';		//writer_ancora_get_theme_option('blog_counters');
						require writer_ancora_get_file_dir('templates/_parts/counters.php');
					}
					?>
					</div>
				</div><!-- .entry-footer -->
